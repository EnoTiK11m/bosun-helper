...
Остальные вкладки получают snapshot через `BroadcastChannel`. Если координация
недоступна, вкладка переходит в безопасный локальный режим и периодически
пытается восстановить совместную работу.
...
Временная точечная диагностика перехода `age → 1 alerts` выводит не более 20
записей с prefix `[BosunHelper][single-alert-age-problem]`. В лог попадают hashes
Subject/group key и короткие метаданные, но не полный snapshot и не DOM nodes.
Она отключается одной строкой: `SINGLE_ALERT_AGE_DEBUG = false` в
`single-alert-age.js`.
